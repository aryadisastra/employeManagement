...
<div class="card">
    <div class="card-header border-bottom">
        <h4 class="card-title">Form Detail/Edit Employee</h4>
    </div>
    <div class="card-body py-2 my-25">
        ...
        <form class="validate-form mt-2 pt-50" method="POST" action="/employee/update/{{$data->id}}" enctype="multipart/form-data">
            @csrf
            <div class="row">
                <div class="col-12 col-sm-6 mb-1">
                    <label class="form-label" for="nama">Nama Lengkap</label>
                    <input type="text" class="form-control" id="nama" name="nama" placeholder="Masukan Nama Lengkap" value="{{$data->nama}}"/>
                </div>
                <div class="col-12 col-sm-6 mb-1">
                    ...
                    <select id="divisi" name="divisi" class="select2 form-select">
                        ...
                        @foreach ($divisi as $dt)
                            <option value="{{$dt->id}}" {{$dt->id == $data->divisi_id ? 'selected' : ''}}>{{$dt->nama}}</option>
                        @endforeach
                    </select>
                </div>
                <div class="col-12 col-sm-6 mb-1">
                    ...
                    <select id="jabatan" name="jabatan" class="select2 form-select">
                        ...
                        @foreach ($jabatan as $dt)
                            <option value="{{$dt->id}}" {{$dt->id == $data->jabatan_id ? 'selected' : ''}}>{{$dt->nama}}</option>
                        @endforeach
                    </select>
                </div>
                <div class="col-12 col-sm-6 mb-1">
                    ...
                    <select id="leader" name="leader" class="select2 form-select">
                        ...
                        @foreach ($lead as $dt)
                            <option value="{{$dt->id}}" {{$dt->id == $data->leader_id ? 'selected' : ''}}>{{$dt->nama}}</option>
                        @endforeach
                    </select>
                </div>
                <div class="col-12 col-sm-6 mb-1">
                ...
                <div class="col-12 col-sm-6 mb-1">
                    ...
                    <input type="text" class="form-control" id="kontak" name="kontak" value="{{$data->kontak}}" placeholder="Masukan Nomor HP" />
                </div>
                <div class="col-12 col-sm-6 mb-1">
                    <label class="form-label" for="tempatlahir">Tempat Lahir</label>
                    <input type="text" class="form-control account-number-mask" id="tempatlahir" value="{{$data->tempat_lahir}}" name="tempat_lahir" placeholder="Tempat Lahir" />
                </div>
                <div class="col-12 col-sm-6 mb-1">
                    <label class="form-label" for="tanggallahir">Tanggal Lahir</label>
                    <input type="date" class="form-control" id="tanggallahir" value="{{$data->tanggal_lahir}}" name="tanggal_lahir" />
                </div>
                <div class="col-12 col-sm-6 mb-1">
                    <label class="form-label" for="tanggalbergabung">Tanggal Bergabung</label>
                    <input type="date" class="form-control" id="tanggalbergabung" value="{{$data->tanggal_bergabung}}" name="tanggal_bergabung"/>
                </div>
